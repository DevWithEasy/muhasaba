...
const quranPath = FileSystem.documentDirectory + "app_dir/quran_data.json";
...
export default function QuranRead() {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const fileInfo = await FileSystem.getInfoAsync(quranPath);
      if (fileInfo.exists) {
        const fileContent = await FileSystem.readAsStringAsync(quranPath);
        const jsonData = JSON.parse(fileContent);
        setData(jsonData);
      } else {
        // ডেমো ডেটা - আপনার ডেটা স্ট্রাকচার অনুযায়ী
        setData([
          { date: "2025-09-02", read: { count: 5, status: true } },
          { date: "2025-09-01", read: { count: 1, status: true } },
          { date: "2025-09-05", read: { count: 3, status: true } },
          { date: "2025-09-07", read: { count: 0, status: false } },
          { date: "2025-09-10", read: { count: 2, status: true } },
          { date: "2025-09-15", read: { count: 4, status: true } },
          { date: "2025-09-20", read: { count: 0, status: false } },
          { date: "2025-09-25", read: { count: 6, status: true } },
          { date: "2025-08-30", read: { count: 3, status: true } },
        ]);
      }
    } catch (error) {
  ...
  // মাসের মোট কুরআন পড়ার দিন গণনা
  ...
  // মাসের মোট কুরআন পড়ার পরিমাণ গণনা
      ...
      <View style={styles.summaryContainer}>
        <View style={styles.summaryItem}>
          ...
          <Text style={styles.summaryValue}>
            {previousMonthTotalDays} দিন
          </Text>
          <Text style={styles.summaryCount}>
            {previousMonthTotalCount} পৃষ্ঠা
          </Text>
        </View>
        <View style={styles.summaryItem}>
          <Text style={styles.summaryLabel}>
          ...
          <Text style={styles.summaryValue}>
            {currentMonthTotalDays} দিন
          </Text>
          <Text style={styles.summaryCount}>
            {currentMonthTotalCount} পৃষ্ঠা
          </Text>
        </View>
      </View>

      <Text style={styles.subtitle}>
        {getMonthName(selectedMonth)} {selectedYear} এর কুরআন পড়ার পরিসংখ্যান
      </Text>
