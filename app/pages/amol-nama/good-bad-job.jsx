...
const filepath = FileSystem.documentDirectory + "app_dir/good_bad_job_data.json";
...
export default function GoodBadJob() {
  ...
  // মাস ও বছরের ভিত্তিতে ডেটা ফিল্টার করা
  ...
  // মাসের মোট ভালো ও খারাপ কাজ গণনা
  const getMonthlyTotals = (month, year) => {
    const monthlyData = filterDataByMonthYear(month, year);
    let goodTotal = 0;
    let badTotal = 0;
    monthlyData.forEach((item) => {
      goodTotal += item.count.good_job || 0;
      badTotal += item.count.bad_job || 0;
    });
    return { goodTotal, badTotal };
  };
  ...
  // মাসের সমস্ত দিনের ডেটা তৈরি করা
  const generateAllDatesData = () => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      if (dateString === todayFormatted) {
        ...
        let todayTotal =
          (existingData?.count.good_job || 0) - (existingData?.count.bad_job || 0);
        let yesterdayTotal =
          (yesterdayData?.count.good_job || 0) - (yesterdayData?.count.bad_job || 0);
      ...
      allDates.push({
        date: dateString,
        counts: existingData ? existingData.count : { good_job: 0, bad_job: 0 },
        total:
          (existingData?.count.good_job || 0) + (existingData?.count.bad_job || 0),
        comparison,
  ...
  const { goodTotal: currentGood, badTotal: currentBad } = getMonthlyTotals(selectedMonth, selectedYear);
  const { month: prevMonth, year: prevYear } = getPreviousMonthYear();
  const { goodTotal: prevGood, badTotal: prevBad } = getMonthlyTotals(prevMonth, prevYear);
  ...
  return (
    <View style={styles.container}>
      ...
      {/* মাসের মোট ভালো ও খারাপ কাজ */}
      <View style={styles.summaryContainer}>
        <View style={styles.summaryItem}>
          <Text style={styles.summaryLabel}>
            {getMonthName(prevMonth)} {prevYear} ভালো কাজ:
          </Text>
          <Text style={styles.summaryValue}>
            {prevGood.toLocaleString("bn-BD")} বার
          </Text>
        </View>
        <View style={styles.summaryItem}>
          <Text style={styles.summaryLabel}>
            {getMonthName(prevMonth)} {prevYear} খারাপ কাজ:
          </Text>
          <Text style={styles.summaryValue}>
            {prevBad.toLocaleString("bn-BD")} বার
          </Text>
        </View>
        <View style={styles.summaryItem}>
          <Text style={styles.summaryLabel}>
            {getMonthName(selectedMonth)} {selectedYear} ভালো কাজ:
          </Text>
          <Text style={styles.summaryValue}>
            {currentGood.toLocaleString("bn-BD")} বার
          </Text>
        </View>
        <View style={styles.summaryItem}>
          <Text style={styles.summaryLabel}>
            {getMonthName(selectedMonth)} {selectedYear} খারাপ কাজ:
          </Text>
          <Text style={styles.summaryValue}>
            {currentBad.toLocaleString("bn-BD")} বার
          </Text>
        </View>
      </View>

      <Text style={styles.subtitle}>
        {getMonthName(selectedMonth)} {selectedYear} এর কাজের পরিসংখ্যান
      </Text>
            ...
            <View key={index} style={[styles.listItem, item.isToday && styles.todayItem]}>
              <View style={styles.dateContainer}>
                ...
                <Text style={[styles.countText, item.isToday && styles.todayCountText]}>
                  মোট: {item.total.toLocaleString("bn-BD")} বার
                </Text>
              </View>
              {/* ভালো ও খারাপ কাজের বিস্তারিত */}
              <View style={styles.jobDetails}>
                <View style={styles.jobDetailItem}>
                  <Text style={styles.jobDetailLabel}>ভালো কাজ:</Text>
                  <Text style={styles.jobDetailCount}>{item.counts.good_job.toLocaleString("bn-BD")} বার</Text>
                </View>
                <View style={styles.jobDetailItem}>
                  <Text style={styles.jobDetailLabel}>খারাপ কাজ:</Text>
                  <Text style={styles.jobDetailCount}>{item.counts.bad_job.toLocaleString("bn-BD")} বার</Text>
                </View>
              </View>
